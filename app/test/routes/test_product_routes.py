...
def test_update_product_route(db_session, product_on_db):
    ...
    product_on_db.name == 'Update camisa'
    product_on_db.slug == 'update-camisa'
...
def test_update_product_route_invalid_id():
    ...
    response = client.put(f'product/update/2', json=body)

    assert response.status_code == status.HTTP_404_NOT_FOUND


def test_delete_product_router(db_session, product_on_db):
...
def test_delete_product_router_invalid_id(product_on_db):
...
def test_list_products_route(products_on_db):
    response = client.get('/product/list?page=1&size=3')
...
def test_list_products_route_search(products_on_db):
